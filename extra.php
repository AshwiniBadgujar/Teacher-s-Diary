...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
 if(isset($_POST['flag1']))
				{
				$abc=1;
					mysqli_query($con,"INSERT into io (tid,dt,tin,tout) values ('$_SESSION[t_id]','$_POST[dat]','$_POST[From_time]','$_POST[Till_time]')"); 				
	 		 	}	
				
if(isset($_POST['ashu']))
		{
$cheks=implode("','",$_POST['checkbox']);
$sql="delete from ioid WHERE ioid in ('$cheks')";
...
<link href="css/extra_style.css" rel="stylesheet" type="text/css">
</head>
...
<div class="side_nav">
    ...
    <a href="extra.php"><div class="side_option2">
    	<div class="side_opimg"  style="background:#00eda6; width:5px; padding-left:0px;">
        	<img src="images/ico/extraico.png" class="s_opimg"  style="margin-left:20px;">
        </div>
        <div class="side_opname"   style="font-size:18px; margin-left:53px;">In / Out Time</div>
    </div></a>
...
<div class="content">
    ...
    <div class="tab_container">
		<div class="f_title">
            ...
            <div class="f_topic">In Time</div>
             <div class="f_topic">Out Time</div>            
        </div>
        
       <?php
               		    $cntc="SELECT * FROM io WHERE tid='$_SESSION[t_id]' order by dt desc";
								...
								while($row3=mysqli_fetch_array($r))
									{	
										echo'<div class="fc_content">
            								...
            								<div class="fc_topic">'.$row3['tin'].'</div>
           									<div class="fc_topic">'.$row3['tout'].'</div></div>';
									}
							}
                ?>
...
<div id="target-content">
	<a href="" class="close"></a>
  	<div id="target-inner">
    	<div class="add_label">Add a New Record</div>
        <hr>
        <form method="POST" action="extra.php">
         <input type="hidden" name="flag1" value="01">
            ...
            <br><br>
            <center>
            <input class="submit" type="submit" value="Add Log">
        	</center>	
        </form>
  	</div>
</div>
...
<div id="target-content2">
    ...
    <div class="add_label">Search Record by Date to Delete</div>
        <hr><center>
        <form method="POST" action="extra.php#target-content2">
        	<br>
         <form method="POST" action="extra.php#target-content2">
        	<div class="sub_name">Enter Date </div>
        ...
        <div class="fp_title">
            ...
            <div class="fp_topic">In Time</div>
             <div class="fp_topic">Out Time</div>         
        </div>
          <div style="opacity:<?php echo'$abc'; ?>">
  <?php     
		if($_SERVER['REQUEST_METHOD'] == 'POST')
		 {
         $resu=mysqli_query($con,"SELECT * FROM io WHERE tid='$_SESSION[t_id]' and dt='$_POST[date]'");			
					    if(mysqli_num_rows($resu)>0)
							{
								while($row6=mysqli_fetch_array($resu))
									{
 										echo'<div class="fcp_content" style="margin-left:-20px;">
        	<div class="rad"><input type="checkbox" name="checkbox[]" id="checkbox[]" value="'.$row6['ioid'].'">			</div>
        	<div class="fcp_name">'.$row6['dt'].'</div>
            <div class="fcp_time">'.$row6['tin'].'</div>
            <div class="fcp_name2">'.$row6['tout'].'</div>
            </div>';
			}
}
																	}?>
